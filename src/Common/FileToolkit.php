...
class FileToolkit
{
    public static function mungeFilename($fileName, $extensions)
    {
        ...
        $newFilename    = array_shift($fileNameParts);
        ...
        return $newFilename.'.'.$finalExtension;
    }
    ...
    public static function moveFile($originFile, $targetGroup)
    {
        ...
        return $originFile->move($targetGroup, $filename);
    }
    ...
    public static function autoCropImages($filePath, $options)
    {
        $pathinfo = pathinfo($filePath);
        $imagine  = new Imagine();
        $rawImage = $imagine->open($filePath);
        ...
        if (!empty($options["imgs"]) && count($options["imgs"]) > 0) {
            foreach ($options["imgs"] as $key => $value) {
                $savedFilePath   = "{$pathinfo['dirname']}/{$pathinfo['filename']}_{$key}.{$pathinfo['extension']}";
                $image           = static::crop($rawImage, $savedFilePath, $options['x'], $options['y'], $options['w'], $options['h'], $value[0], 1.0 * $value[0] / $naturalSize->getWidth() * $naturalSize->getHeight());
                $filePaths[$key] = $savedFilePath;
            }
        } else {
            $savedFilePath = "{$pathinfo['dirname']}/{$pathinfo['filename']}.{$pathinfo['extension']}";
    ...
    public static function reduceImgQuality($fullPath, $level = 10)
    {
        ...
        try {
            $imagine = new Imagine();
            $imagine->open($fullPath)->save($fullPath, $options);
        } catch (\Exception $e) {
    ...
    {
        $dir = substr($savePath, 0, strrpos($savePath, '/'));
        if (!file_exists($dir)) {
            mkdir($dir);
        }
        $curl = curl_init($url);
